---
title: PITFALLS.MD -- DEPRECATED
type: meta
tags: [meta, debug]
updated: 2026-05-26
description: DEPRECATED 2026-05-26 -- tartalom átkerült az érintett skill §6 Hibakezelés szekciókba. Ez a fájl archív referencia, ne töltsd be hibaelhárításhoz.
---

> ⚠️ **DEPRECATED (2026-05-26):** A pitfalls.md tartalma átkerült a skill-ek `§6 Hibakezelés` szekciókba.
> Hibaelhárításhoz az adott lépés skill fájlját olvasd (pl. `02_nlm_notebook_setup.md §6`, `04_nlm_query_runner.md §6`).
> Ez a fájl archív referencia -- ne töltsd be session elején.
...
## 3.4. 03-1_qfig_parser: Markdown bold formátum + hiányzó TÉMAKÖR

**Tünet:** `parse_qfig_text()` 0 entry-t ad vissza; `keywords: []` minden catalog bejegyzésen.

**Gyökérok:** (1) Az NLM a Qfig outputot Markdown-bold listában adja vissza (`*   **Forrás:**`) -- az eredeti FIELD_RE csak sima szöveget (`FORRÁS: ...`) várt. (2) A zárójel `:**` sorrendű (MEZŐ: _érték_), nem `**:**`. (3) Az NLM nem generál TÉMAKÖR mezőt -- a keywords mező üres marad.

**Megoldás (`03-1_qfig_parser.py` v2, 2026-05-26):**
1. FIELD_RE: `r'^\*?\s*\*{0,2}(MEZŐ)\s*:\*{0,2}\s*(.*)'` -- bullet + opening `**` prefix, majd `:**` (colon-first) zárás.
2. FIELD_MAP: `felirat` → `caption`; `abra szama`/`ábra száma` → `num`.
3. `_keywords_from_text()`: TÉMAKÖR hiányában LEÍRÁS+FELIRAT szövegéből auto-extrakció (stopword-szűrés, max 8 szó).

**Státusz:** ✅ Javítva 2026-05-26.
