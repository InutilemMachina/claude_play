...
- 🔲 TODO: **NLM Studio mentés automatizálása nem lehetséges CLI-n (tesztelve 2026-05-30).** `nlm studio` CLI csak `status`, `delete`, `rename` parancsot ismer -- `create`/`save` nem elérhető. Egyetlen automatizálási út: **Claude in Chrome MCP** (böngésző-automatizáció: NLM chat UI-ban query futtatása → "Mentés Studio-ba" gomb kattintása). Jelenlegi állapot: 😎 manuális lépés. Prioritás: közepes -- a `3_raw_outputs/nlm_q*.txt` fájlok a pipeline számára elegendők; a Studio mentés csak auditálhatóság/reprodukálhatóság szempontjából értékes.
...
- ✅ **Q1 bevezető-prompt implementálva (J3, 2026-05-29).** A root-query (L0, parent=None) most rövid 2-3 bekezdéses bevezető szöveget kér, nem teljes áttekintést. Eredmény: Q1 1213 kar (volt 5269 kar, -77%). Az eredeti `"Beszeljen az ezekben a forrasokban targyalt {root} temakorrol."` prompt teljes áttekintést produkált → azonnali ismétlést hozva az összes többi query-vel. Kanonikus prompt: `scripts/04_nlm_dfs_queries.py build_query()`, `parent is None` ág.
...
- ✅ **`--max-level 2` BSc-re ajánlott és tesztelve (2026-05-29, mini teszt iteráció 2).** L0+L1+L2 (23 query) vs. L0-L4 (37 query). Hatás: bullet-arány 85%→40%, próza 15%→59%, tartalmi ismétlés ~40%→~15%. Gyökérok: L3+ csomópontok (pl. Planck, Stefan-Boltzmann részletek) az L2 szekció tartalmát redundánsan ismétlik. BSc kurzusoknál az L2 mélység elegendő. Ajánlott default: `--max-level 2`; MSc/haladó: `--max-level 3`. Megjegyzés: `dfs_node_list.json` automatikusan generálódik minden futáskor, az assembler ebből tudja az egyes queryek szintjét.
