...
- ✅ **WIP prezentáció flow javítva (2026-05-30, mini2).** Helyes sorrend: (1) Claude olvassa a `4_wip_outputs/N_Jegyzet.md`-t és generálja a `4_wip_outputs/N_Prezentacio.md`-t (Marp, Claude feladata); (2) `12_pptx_gyarto.py --week-dir` konvertálja PPTX-re; (3) `--pdf` flag PDF másolatot is generál (PowerPoint COM). Korábbi hiba: `due_fill.py --week-dir` direktben hardcoded demo-tartalmat generált, nem olvasott a Jegyzetből.
- ⚠️ WARNING: **`due_fill.py` CLI-módja DEMO tartalmat generál** (Biofizika placeholder) — NEM alkalmas pipeline-futtatásra önmagában. Csak a Python API (`DUEPresentation`) való programmatikus hívásra használható. A `12_pptx_gyarto.py --week-dir` a helyes CLI pipeline-parancs.
