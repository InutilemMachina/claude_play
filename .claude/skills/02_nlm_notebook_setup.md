...
- ⚠️ WARNING: **Prompt B egy-soros bypass-nál `VALASZOLJ KIZAROLAG MAGYARUL` az első sor legyen (tesztelve 2026-05-30, mini2).** Enélkül az NLM angolul válaszol, még ha a forrásanyagok és a query magyarul érkeznek is. A teljes Prompt B tartalmaz nyelvi instrukciókat, de a bypass egy-soros verzióból ez kimaradhat.
- ⚠️ WARNING: **Prompt B multi-line string PS 5.1-ben soronként adódik át natív CLI-nek** (`Got unexpected extra arguments`). Temp fájl + `Get-Content -Raw` sem segít -- a newline-ok argumentum-határként értelmezödnek. Bypass: egy-soros prompt (newline-ok nélkül). Következmény: a teljes Prompt B csak NLM UI-ban illeszthető be manuálisan. (Tesztelve 2026-05-30, mini2.)
- 🔲 TODO: **Forrásnyelv-ellenőrzés a 02 checkpoint-nál.** Ha a feltöltött források nem magyarok (pl. angol PDF-ek, nem-magyar weblapok), az NLM válaszok valószínűleg nem magyar nyelvűek lesznek, és a mindmap is angolul generálódik. Ilyenkor a user-t figyelmeztetni kell: (1) ellenőrizze a mindmap nyelvét az NLM Studio-ban; (2) ha angol, fontolja meg magyar nyelvű kiegészítő forrás hozzáadását, vagy fogadja el az angol mindmap-et (a DFS queries-ek akkor is futnak). A figyelmeztetés helye: a 02 checkpoint szövege a pipeline.md §4-ben.
- 🔲 TODO: **Általános NLM UI copy-paste útmutató hiányzik.** A pipeline feltételezi, hogy a user tudja, mit hova kell beilleszteni az NLM UI-ban. Szükséges egy kanonikus lista (02 skill §3 vagy pipeline.md §5): Prompt B ékezetes változat → NLM Configure Chat → Custom Instructions; Prompt C.1/C.3/C.4 → NLM Studio → Data Tables; mindmap export → Ultra Explorer bővítmény → menteni `3_raw_outputs/nlm_mindmap_export.md`-ként.
- 🔲 TODO: **NLM forráspanel parse-hiba nincs detektálva (tesztelve 2026-05-30, mini2).** Az NLM UI-ban egyes URL-források piros jelzéssel jelennek meg (parse/fetch sikertelen), de a CLI `nlm source add` vagy sikeres kóddal tér vissza, vagy hibát dob — sem esetben nem ellenőrzi a forrás NLM-oldali feldolgozási státuszát. Következmény: a `citations_seed.json`-ban szereplő `nlm_uuid` "érvényesnek" látszik, de a forrás tartalmilag üres az NLM-ben. Megoldás: `nlm source list <NB_ID>` futtatása a 02 checkpoint-nál, és a `status != ready` források kiszűrése + figyelmeztetés. (Konkrét eset: `https://thermadelta.hu` — a CLI hibát dobott, de az NLM mégis regisztrálta, és piros forrásként jelenik meg.)
- 🔲 TODO: **Retroaktív forrás-hozzáadás protokollja hiányzik.** Ha a user 02 után helyez el új forrást az `1_raw_inputs/`-ban: (1) fájlnév-ellenőrzés + átnevezés; (2) eredeti URL visszakeresése HTML fejlécből (ha nincs publikus URL, NLM-be nem tölthető); (3) `nlm source add --url` a meglévő notebookban; (4) `citations_seed.json` retroaktív bővítése. Dokumentálandó: 01 skill §6 + 02 skill §3. (Tesztelve 2026-05-30, mini2, thermadelta2026_webpage.html.)
